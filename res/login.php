<?php /* CoreProtect LWI - v0.8.0-beta * Login Script by richcheting from http://www.ricocheting.com/code/php/simple-login */require_once __DIR__."/../settings.php";define('LOGIN_USER',$_login["username"]);define('LOGIN_PASS',$_login["password"]);define('LOGIN_DURATION',$_login["duration"]*86400);class Login{var $prefix="login_",$user="",$pass="";function authorize(){
  if(isset($_COOKIE[$this->prefix.'user'])){$_SESSION[$this->prefix.'user']=$_COOKIE[$this->prefix.'user'];$_SESSION[$this->prefix.'pass']=$_COOKIE[$this->prefix.'pass'];}if(isset($_POST['action'])&&$_POST['action']=="set_login"){$this->user=$_POST['user'];$this->pass=md5($this->prefix.$_POST['pass']);$this->check();if(isset($_POST['remember'])){setcookie($this->prefix."user",$this->user,time()+($this->cookie_duration*86400));setcookie($this->prefix."pass",$this->pass,time()+($this->cookie_duration*86400));}$_SESSION[$this->prefix.'user']=$this->user;$_SESSION[$this->prefix.'pass']=$this->pass;}elseif(isset($_GET['action'])&&$_GET['action']=="prompt"){session_unset();session_destroy();if(!empty($_COOKIE[$this->prefix.'user']))setcookie($this->prefix."user",NULL,-1,"/");if(!empty($_COOKIE[$this->prefix.'pass']))setcookie($this->prefix."pass",NULL,-1,"/");$this->prompt();}elseif(isset($_GET['action'])&&$_GET['action']=="clear_login"){session_unset();session_destroy();if(!empty($_COOKIE[$this->prefix.'user']))setcookie($this->prefix."user",NULL,-1,"/");if(!empty($_COOKIE[$this->prefix.'pass']))setcookie($this->prefix."pass",NULL,-1,"/");$msg='Logout successful.';$this->prompt($msg);}elseif(!isset($_SESSION[$this->prefix.'pass'])||!isset($_SESSION[$this->prefix.'user'])){$this->prompt();}else{$this->user=$_SESSION[$this->prefix.'user'];$this->pass=$_SESSION[$this->prefix.'pass'];$this->check();}}function check(){if(md5($this->prefix.LOGIN_PASS)!=$this->pass||LOGIN_USER!=$this->user){if(!empty($_COOKIE[$this->prefix.'user']))setcookie($this->prefix."user",NULL,-1,"/");if(!empty($_COOKIE[$this->prefix.'pass']))setcookie($this->prefix."pass",NULL,-1,"/");session_unset();session_destroy();$msg='Incorrect username or password.';$this->prompt($msg);}}function prompt($msg=''){?><!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
